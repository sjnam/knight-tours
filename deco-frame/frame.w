...
$1$이기만 하면 어떤 직사각형이든 짓는다. 나이트 이동은 $90^\circ$ 회전에 대해 불변이라
진짜 닫힌 투어다. 그래서 이 프로그램은 명령행이 시킨 크기로 액자를 {\it 하나만\/} 짓는다.
기본값은 $103\times73$(A4 비율)이고, 그것을 limbo의 \.{\\plainoutput}이 이 문서의
{\it 모든 페이지\/} 바탕에 깐다.
...
@ 여기서 degree $2$와 고리 수 사이를 짚어 두자. 어떤 칸의 degree란 거기 붙은 이음의
개수다. 닫힌 투어에서 나이트는 칸마다 한 번 들어와 한 번 나가므로 모든 칸의 degree가
정확히 $2$다. 그러니 degree $2$는 투어의 {\it 필요조건\/}이다.

충분조건은 아니다. 모든 칸이 degree $2$인 이음 집합은 하나의 큰 고리일 수도, 여러 개의
작은 고리일 수도 있다---그래프 이론에서 2-factor라 부르는 것이다. 들어가며에서 말한 네
고리가 바로 그 짝이다. 매듭 하나만 네 모서리에 돌려 놓고 $103\times73$을 지으면
$1020$칸이 {\it 모두\/} degree $2$인데도 고리는 넷으로 갈린다. 크기는 $106$, $404$,
$404$, $106$---긴 변 둘과 짧은 변 둘이 저마다 자기 고리다. 나이트가 위쪽 변을 뱅뱅 돌
뿐 오른쪽 변으로 넘어가지 못한다. |loopID|가 degree와 고리 수를 {\it 둘 다\/} 보는
까닭이 이것이다.

@ 그 네 고리를 하나로 꿰는 데 쓴 것이 2-opt다. 원래 외판원 문제를 푸는 고전적 개선
수법으로(Croes, 1958), 이름 그대로 이음 {\it 둘\/}을 바꿔치우는 동작이다. 이음 $a\,b$와
$c\,d$를 골라 지우고, 끝점을 엇갈리게 $a\,c$와 $b\,d$로 다시 잇는다.
\smallskip
\centerline{\pic width 11.5cm{twoopt-1.pdf}}
\smallskip
...
새로 만드는 $a\,c$와 $b\,d$가 {\it 둘 다 진짜 나이트 이동\/}이어야 한다. 그래서 아무
