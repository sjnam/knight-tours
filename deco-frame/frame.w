...
두르는 배경으로 쓰려 한다. 그의 별무늬로 {\it 새로운 크기의 직사각\/} 액자를 짓되,
그것 역시 진짜 하나의 닫힌 투어가 되게 한다.
...
@ 이 프로그램은 명령행으로 액자의 세로와 가로의 길이를 $N_h$, $N_w$의 매개변수로 받는다.
마찬가지로 뒤에서 밝혀지지만 각 변이 $13$ 이상이면서 $6$으로 나눈 나머지가 $1$이기만 하면
어떤 직사각형이든 만들어 낸다. 명령행 액자 크기의 기본값은 $103\times73$(A4 비율)이고,
이 문서 자체가 그 액자를 사용하는 한 예가 되는데, 만든 액자를 곧바로 이 문서의
{\it 모든 페이지\/} 바탕에 깐다. 프로그램의 뼈대는 다음과 같다.
...
@* 크누스의 마디와 모서리 매듭. 기본 자료형부터 정의하자. 칸은 $(r,c)$로 적고,
...
이음들이다. 첫번째 이음 |knuthMod|는 곧은 변의 주기 6 마디(열여덟 이음)로,
위쪽 변의 띠 좌표(행 $0,1,2$)로 적었다. 열이 $6$을 넘는 이음은 다음 마디로 이어진다.
아래 그림은 이 표를 여섯 칸씩 옮겨 세 벌 이어 깐 것인데, 가운데 빨간 한 벌이 표에 적힌
바로 그 열여덟 이음이다. 별이 촘촘히 맞물린 무늬가 마디 하나에서 나온다. 이 그림은
같은 디렉토리에 있는 \.{frame.mp}가 그린다---액자와 달리 마디의 좌표는 크기에 따라
...
@ 왜 하필 열여덟인가? 출발점과 도착점이 같은 닫힌 투어에서는 모든 칸의 차수가 $2$이므로
차수의 합이 $2V$이면서 $2E$다. 곧 {\it 이음 수와 칸 수가 같다\/}. 마디는 곧은 변을 따라
여섯 칸 주기로 되풀이되니 한 주기가 품는 칸은 $6\times3=18$이고, 무한히 뻗은 띠에서도
$E=V$는 주기마다 성립하므로 한 마디의 이음도 열여덟이다. |knuthMod|의 항목이 열여덟인
것은 우연이 아니라 $6\times3$의 결과다. 같은 셈으로 폭 3칸 테두리 전체의 이음 수도 칸 수와
...
전시장을 설명해 놓은 크누스의 글을 자세히 읽었다면, 그리고 그에 따라 이 책을 진작에 찾아봤다면,
액자 이미지를 분석하는 일 따위는 하지 않았어도 되었을 것이다. 하지만 괜찮다. 그 성가신 일은 앞서
밝힌 대로 AI가 했으니까. 덧붙이자면 8층에도 프리즈가 따로 있고(모티프
`\.{133-122-011-023-131-110-231-223-001-101}', $3\times(10n+20)$ 고리)
그것은 엘리베이터 {\it 문 위\/}에 있으니, {\it 왼쪽 벽\/}의 겹친 액자와 헷갈리지 말 일이다.

@ 꼼꼼하지 못한 성격 탓에 AI를 고생시킨 일에 대한 후회는 그만 접어두고, 두번째 이음들을 살펴보자.
그 이음들은 바로 |knuthCorners|로써 모서리를 도는 매듭 넷(저마다 서른 이음)이다.
두 변의 마디를 모서리에서 이어 주며, 모서리 칸을 원점으로 한 좌표로 적었다. 크누스의
$55\times55$ 투어에서 마디를 걷어 낸 나머지를 네 모서리별로 갈라 얻은 것인데,
$31\times31$에서 얻은 것과 글자 하나 다르지 않았다---즉, 크기와 무관한 그의 재료다.
@<타입...@>=
...
@ 네 귀퉁이 가운데 $0$번 모서리 매듭이 기본꼴이다. $1$번과 $2$번은 서로 같고 기본꼴과 이음 다섯이 다르며,
...
$N=31$이면 둘 사이가 넉넉히 벌어져 그 틈을 마디가 메운다. $N=13$이면 $N-8=5$이라
$5,6$ 두 열에서 맞닿는데, 이 겹침은 성하다---두 매듭이 서로 맞물리는 자리일 뿐 칸의 차수를
넘기지 않는다. 그런데 $N=7$이면 뒤엣것이 $-1$열에서 시작해야 하니 판 밖이다. 그렇게 매듭이
...
|loopID|가 차수와 고리 수를 {\it 둘 다\/} 보는 까닭이 이것이다.
...
\medskip
\noindent
첫 줄의 간선 $1020$개가 앞서 표로 세어 둔 $6(103+73)-36$ 그대로이고, 모서리 배치가
$0123$이라는 것은 기본값 $103\times73$에서 크누스 자신의 배치가 그대로 통했다는 뜻이다.
