...
바로 그 열여덟 이음이다. 별이 촘촘히 맞물린 무늬가 마디 하나에서 나온다. 액자 그림은
같은 디렉토리에 있는 \.{frame.mp}가 그린다---액자의 좌표와 달리 선의 굵기나 색상과 같은
재료는 크기에 따라 달라지지 않는 상수라 프로그램이 지을 까닭이 없어 손으로 적어 두었다.
...
@ 두번째 이음들 |knuthCorners|는 모서리를 도는 매듭 넷(저마다 서른 이음)이다.
