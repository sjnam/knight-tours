...
먼저 크누스의 무늬를 그의 \pdfURL{{\it 그림}}%
{https://cs.stanford.edu/\TILDE/knuth/KTf.jpg}%
...
차수(degree)가 $2$가 되기는 해도 변마다 고리 하나씩 {\it 네 개의 닫힌 고리\/}로 갈려서,
이웃한 두 고리를 2-opt 알고리즘으로 이어 붙여 하나로 꿰야 했다. 그런데 그렇게 지은 액자를
크누스의 것과 간선 단위로 견주어 보니 모서리가 서로 달랐다. 2-opt가 바꿔치울 수 있는 이음이
하필 모서리에만 있었기 때문인데, 이 ``하필''이 실은 피할 길 없는 것이었음을 뒤에서 자세히 밝힌다.
...
각 변이 $13$ 이상이면서 $6$으로 나눈 나머지가 $1$이기만 하면 어떤 직사각형이든 만들어 낸다.
명령행 액자 크기의 기본값은 $103\times73$(A4 비율)이고, 이 문서 자체가 그 액자를
사용하는 한 예가 되는데, 만든 액자를 곧바로 이 문서의 {\it 모든 페이지\/} 바탕에 깐다.
프로그램의 뼈대는 다음과 같다.
...
바로 그 4층 프리즈의 모티프를 전시장 안내글의 {\it 다른 층\/} 대목---{\sl Level 3:
Long and skinny tours\/}---에서 이렇게 적어 두었다:
\smallskip
{\narrower\narrower\noindent\sl
...
표기법은 그의 책 {\sl Selected Papers on Fun and Games\/} 42장 {\sl Long and
Skinny Knight's Tours\/}에 있다. 책장에서 그 책을 꺼내어 42번째 장을 읽으니 규칙이
또렷했다. 여섯 묶음을 그 규칙대로 풀어 이음으로 펴 보면 아래 표와 한 이음도 어긋나지 않는다.
...
@ 네 귀퉁이 가운데 $0$번 모서리 매듭이 기본꼴이다. $1$번과 $2$번은 서로 같고 기본꼴과 이음
다섯이 다르며, $3$번은 이음 둘이 다르다. 그러니까 실제로는 {\it 세 종류\/}다. 바로 이 어긋남이
네 고리를 하나로 꿰는 건너뜀이라, 매듭을 제자리에 놓기만 하면 2-opt가 필요 없다.
...
함께 지니는 스물다섯을 한 번만 적고 갈리는 자리만 따로 둔다. 두 팔이 기역({\it ㄱ})자로 꺾여
두 변의 마디를 이어 주는 모양이 보인다. 기본꼴은 이렇게 생겼다.
$$
...
$5,11,\ldots$로 깔되 $L-13$에서 멈추므로, 길이 $L=6k+1$인 변이 받는 마디는
$5+6j\le6k-12$를 푼 $k-2$개다. 네 변을 합하면 $2(p+q)-8$개. 매듭 하나의 이음 수를
$C$라 하면 $$18\bigl(2(p+q)-8\bigr)+4C=6(N_h+N_w)-36=36(p+q)-24$$
...
$N$인 한 변에는 한 매듭의 가로팔이 $0\ldots6$열을, 이웃 매듭의 세로팔이
$N-8\ldots N-1$열을 차지한다.
...
@ 함수|isBorder|는 주어진 칸(|cl|)이 폭이 3칸인 테두리에 속하는지를 가린다. 판 밖이거나
테두리가 아닌 칸으로 가는 이음은 받지 않는다.
@<보조...@>=
...
이음이 하나도 없다. 세어 보고 안 것이 아니라 표를 들여다보면 그냥 나온다---마디 칸은 행이
많아야 $2$이니 $r<3$이 언제나 참이고, 네 매듭 모두 행이 $3$ 이상인 칸은 열이 많아야
$2$이니 이번에는 $c<3$이 언제나 참이다. 크기가 얼마든 상관이 없다. 재료가 애초에
...
넘어간다.
그래서 길이 $L$인 한 변은 {\it 두 칸씩 맞물린 사슬\/}이 된다. $L=31$이면 이렇다---위층은
재료가 저마다 덮는 열을 막대로 늘어놓은 것이고, 아래층은 바로 그 자리에 놓이는 실제
이음이다. 두 층은 축척이 같으니 막대와 이음이 열 단위로 그대로 포개진다.
$$\pic{frame-7.pdf}$$
...
아래층은 시늉이 아니라 진짜다. $31\times31$ 액자를 실제로 지어 그 자리를 오려 낸 것과
이음 하나까지 같다($117$개). 두 팔이 아래로 뻗은 것은 매듭의 나머지 반쪽이 이웃 변의
몫이기 때문이고, 거꾸로 오른팔 끝의 이음 셋은 그 이웃 변의 첫 마디가 이리로 걸쳐 든
것이다. 모서리에서 두 변이 서로의 살림에 손을 얹고 있는 셈이다.
@<네 변에 마디를, 네 모서리에 매듭을 깐다@>=
for i, f := range xf {
...
{\it 하나의 닫힌 투어\/}가 되는 첫 배치를 쓴다. 크누스의 배치가 그대로 통하는 크기는
어림잡아 절반이 아니라 {\it 꼭\/} 절반이다. $N_h=6p+1$로 두었을 때 $p$를 $4$로 나눈
나머지가 $0$이거나 $1$이면 통하고, $2$이거나 $3$이면 통하지 않는다. 뜻밖에도 $N_w$는
아무 상관이 없다. 나머지 절반에서도 $256$가지 안에 통하는 것이 반드시 있다. 왜 그런지는
바로 다음다음 절에 적었다.
@<보조...@>=
...
@ 왜 $4$로 나눈 나머지만 따지면 되는가? 한 변을 통째로 떼어 놓고 보면 답이 나온다.
마디를 $m$개 이어 깐 변은 양 끝에 두 열씩의 {\it 항구\/}를 두고 그 사이를 이음이
가로지른다. 이 변이 바깥에 내보이는 것은 딱 하나다---어느 항구 칸과 어느 항구 칸이
한 줄로 이어지는가 하는 {\it 짝 무늬\/}. 그것을 $P(m)$이라 부르자. 변 안에서 저 혼자
닫혀 버리는 고리는 하나도 없으니, $P(m)$이 그 변에 대해 알아야 할 전부다.

@ 이제 마디를 하나 더 깐다고 하자. 새 마디는 앞서 깔아 둔 것들과 {\it 오른쪽 항구에서만\/}
만난다. 나이트 행마가 두 열을 넘지 못하는데 조각끼리는 꼭 두 열씩 겹치는 덕이다. 그러니
$P(m+1)$을 알아내는 데 $P(m)$ 말고는 아무것도 필요하지 않다---$P(m+1)=G(P(m))$인 셈이다.
$G$가 어떻게 생겼는지는 몰라도 좋다. 중요한 것은 $P$가 {\it 점화식\/}을 따른다는 사실
하나뿐이다.

$m=1$부터 $9$까지 $P$를 실제로 구해 보면 $P(m+4)=P(m)$이다. 점화식을 따르는 수열은 한 번
제자리로 돌아오면 그 뒤로 영영 되풀이하니, 이것으로 $m\ge1$인 모든 $m$에서 주기가 $4$임이
정해진다. 길이 $L=6k+1$인 변에는 마디가 $k-2$개 깔리므로(앞서 본 그대로다), $m$의 주기
$4$가 곧 $p$와 $q$의 주기 $4$다.

구멍이 하나 남는다. $m=0$, 곧 변이 $13$칸이라 마디가 하나도 없는 경우다. 그때는 양 끝
항구가 같은 열에 포개져 $P(0)$이 아예 말이 되지 않는다. 그래도 곤란하지 않다. 한 변이
$13$이어도 {\it 다른\/} 축의 주기는 멀쩡히 살아 있고, 두 변이 다 $13$인 것은
$13\times13$ 하나뿐이라 직접 보면 그만이다.

그리하여 배치가 통하느냐 마느냐는 오로지 $(p\bmod4,\ q\bmod4)$에 달렸다. 경우가 열여섯
가지뿐이니 작은 크기 몇 개만 훑으면 모든 크기를 덮는다. $p$와 $q$를 $2$부터 $10$까지
($81$가지) 훑어 열여섯 가지가 다 나오고 서로 어긋나는 데가 없음을 보았다.

여기서 뜻밖의 선물이 하나 나온다. 열여섯 경우 {\it 전부\/}에서 통하는 배치를 찾아보면
여덟이 나오는데, 매듭 $1$번과 $2$번이 같은 꼴임을 떠올리면 실은 넷이다.
$$\hbox{\.{0133}\qquad\.{3013}\qquad\.{3301}\qquad\.{1330}}$$
넷을 나란히 놓고 보면 웃음이 난다. 같은 것을 한 자리씩 돌려 놓았을 뿐이다. 어느 모서리에서
세기 시작했느냐가 다를 뿐 {\it 배치는 하나\/}다. 크누스의 \.{0123}과 달리 이것은 크기를
가리지 않는다.

그러니 |ringTour|의 탐색은 반드시 성공한다. 그 끝에 세워 둔 |log.Fatalf|는 닿을 수 없는
줄이다. 그래도 지우지 않고 둔다---매듭 표를 잘못 건드리는 날에는 저기에 닿을 테니까.
...
수법으로(Croes, 1958), 이름 그대로 이음 {\it 둘\/}을 바꿔치우는 동작이다. 이음
$a\adj b$와 $c\adj d$를 골라 지우고, 끝점을 엇갈리게 $a\adj c$와 $b\adj d$로
다시 잇는다.
$$\pic{frame-2.pdf}$$
...
새로 만드는 $a\adj c$와 $b\adj d$가 {\it 둘 다 진짜 나이트 이동\/}이어야 한다.
그래서 아무 데서나 되지 않는다. $103\times73$에서 서로 다른 고리를 잇는 2-opt를 죄다
세어 보면 열두 가지뿐이고, 거기 나오는 칸은 하나도 빠짐없이 모서리에서 여섯 칸 안쪽이다.
...
정의만 담은 \.{framedef.mp}에는 \.{end.}가 없으니 이것을 들여오면 그럴 일이 없다---%
다른 문서가 \.{frame(0.6red, 0.95white, 0.4pt)}처럼 선 색도 배경색도 굵기도 원하는
대로 액자를 다시 쓸 수 있다. \.{expr} 매개변수는 색 타입을 가리지 않으므로 \.{RGB}
세 원소든 \.{CMYK} 네 원소든 그대로 넘어간다.
